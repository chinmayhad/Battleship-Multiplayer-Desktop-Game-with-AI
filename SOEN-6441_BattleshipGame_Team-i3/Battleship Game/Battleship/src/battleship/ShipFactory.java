...
import battleship.AbstractBoard.Ocean;
...
public class ShipFactory {
    ...
    /**
     * Creates a ShipFactory object
     * 
     * @param size the number of spaces the ship takes up
     * @param gameBoard the gameBoard that the ship will be placed on
     */
    public ShipFactory(int size, ArrayList<ArrayList<Ocean>> gameBoard) {
        this.size = size;
        this.hits = 0;
        this.destroyed = false;
        this.generateCoords(gameBoard);
    }
     ...
     */
    public ShipFactory(int size, int startX, int startY, int endX, int endY) {
        this.size = size;
        this.startX = startX;
        this.startY = startY;
        this.endX = endX;
     ...
     */
    public boolean checkCoords(ArrayList<ArrayList<Ocean>> gameBoard) {
        for (int x = this.startX; x <= this.endX; x++) {
            for (int y = this.startY; y <= this.endY; y++) {
                if (x < 0 || y < 0 || x > 9 || y > 9 || (gameBoard.get(x).get(y).getShip() != null)) {
                    return false;
                }
            }
        }
     ...
     * @param gameBoard
     */
    public void generateCoords(ArrayList<ArrayList<Ocean>> gameBoard) {
